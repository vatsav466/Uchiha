...
def get_since_until(  # pylint: disable=too-many-arguments,too-many-locals,too-many-branches,too-many-statements
    ...
    if instant_time_comparison_range:
        ...
        if feature_flag_manager.is_feature_enabled("CHART_PLUGINS_EXPERIMENTAL"):
            time_unit = ""
            delta_in_days = None
            if instant_time_comparison_range == "year":
                time_unit = "YEAR"
            elif instant_time_comparison_range == "month":
                time_unit = "MONTH"
            elif instant_time_comparison_range == "week":
                time_unit = "WEEK"
            elif instant_time_comparison_range == "inherited":
...
class EvalHolidayFunc:  # pylint: disable=too-few-public-methods
    ...
    def eval(self) -> datetime:
        ...
        raise ValueError(
            "Unable to find such a holiday: [%(holiday)s]", holiday=holiday
        )
