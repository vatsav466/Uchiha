...
def create_extraction_log_table():
    """Create lpg_plant_extraction_log table if it doesn't exist"""
    try:
...
def get_extraction_date(plant_name, default_days=5):
    ...
    try:
        ...
        query = """
        SELECT last_extracted_date FROM lpg_plant_extraction_log
        WHERE plant_name = %s
        """
        ...
        if result:
            ...
            insert_query = """
            INSERT INTO lpg_plant_extraction_log
...
def update_extraction_log(plant_name, status, max_date=None):
    ...
    try:
        ...
        if status == "EXTRACTED" and max_date:
            query = """
            UPDATE lpg_plant_extraction_log
...
if __name__=="__main__":
    try:
        print(f'Starting lpg production sync at {urdhva_base.utilities.get_present_time().strftime("%Y-%m-%d %H:%M:%S")}')
        ...
        print(f'Completed lpg production sync at {urdhva_base.utilities.get_present_time().strftime("%Y-%m-%d %H:%M:%S")}')
    except Exception as e:
        print("*-"*25)
