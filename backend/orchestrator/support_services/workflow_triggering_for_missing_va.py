...
async def trigger_workflow_for_missing_data():
    query = f"""select * from alerts 
                where alert_section='VA' and 
                bu in ('LPG','TAS') and alert_status!='Close' and workflow_instance_id = ''
            """
    ...
    for alert in alerts_data.get("data", []):
        ...
        interlock_name = interlock_mapping.get_interlock_name(bu=alert.get('bu'), interlock_name=alert.get('interlock_name'),sop_id=alert.get('sop_id'))
