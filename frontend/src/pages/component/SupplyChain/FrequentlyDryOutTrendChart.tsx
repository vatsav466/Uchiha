...
interface FrequentlyDryOutCount {
  month: string;
  product_code: string;
  frequent_dryout_count: number;
}

interface FrequentlyDryOutData {
  [key: string]: any; // For dynamic columns
}

interface FrequentlyDryOutResponse {
  status: boolean;
  message: string;
  counts: FrequentlyDryOutCount[];
  data: FrequentlyDryOutData[];
}
...
interface FrequentlyDryOutTrendsChartProps {
  filters?: FilterValue[];
}

const FrequentlyDryOutTrendsChart: React.FC<FrequentlyDryOutTrendsChartProps> = ({ filters = [] }) => {
  ...
  const [gridData, setGridData] = useState<FrequentlyDryOutData[]>([]);
  const [filteredGridData, setFilteredGridData] = useState<FrequentlyDryOutData[]>([]);
  ...
  const handleDownloadCSV = () => {
    ...
    link.setAttribute('download', `frequently_dryout_details_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const requestBody = {
        filters: filters,
        action: "frequently_dry_out_trends",
        drill_state: "",
      ...
      const result: FrequentlyDryOutResponse = response.data;
      return result;
    } catch (error) {
  ...
  const processData = (apiResponse: FrequentlyDryOutResponse | null) => {
    ...
    const groupedByMonth = filteredCounts.reduce((acc, curr) => {
      ...
      acc[curr.month][curr.product_code] = curr.frequent_dryout_count;
      return acc;
...
yAxis.children.unshift(
          ...
          <CardHeader className="pb-0 p-1 pt-1">
            <div className="flex flex-col gap-2">
              <div className="flex items-center justify-between">
                <CardTitle className="text-xs font-bold text-gray-800">Frequently DryOut Trends Analysis</CardTitle>
  ...
  export default FrequentlyDryOutTrendsChart;