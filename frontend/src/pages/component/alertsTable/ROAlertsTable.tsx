...
export const ROAlertsTable: React.FC<ROAlertsTableProps> = ({ query, onLocationChange, alertSection, useCreatedAtForCreatedAtColumn = false }) => {
  ...
  const [selectedColumns, setSelectedColumns] = useState<string[]>([
    ...
    'last_notified_to', 'actions', 'zone', 'external_timestamp', 'vehicle_number',"alert_section", "equipment_name"
  ]);
    ...
    try {
      ...
      const excelData = data.map((item: any) => {
        ...
        return {
          ...
          ...(isVTSSection ? { 'Vehicle Number': item.vehicle_number || '' } : {}),
  ...
  const fields = [
    'unique_id', 'sap_id', 'region', 'location_name', 'severity', 'interlock_name',
    'created_at',   'external_timestamp','device_type', 'device_name', 'assigned_to_role', 'last_escalated_to',
    'last_notified_to', 'zone', 'vehicle_number',"alert_section", "equipment_name"
  ];
  ...
  // Finally, update handleRefresh to clear the selected sapId
  const handleRefresh = useCallback(() => {
    // Clear the search text
    setSearchText('');

    // Clear the selected sapId
    setSelectedSapId(null);

    // If onLocationChange is provided, call it with null or empty string to indicate deselection
    if (onLocationChange) {
      onLocationChange('');
    }
    ...
    // },
{
  ...
  valueGetter: (params: any) => {
    ...
    return params.data?.alert_section === 'VA'
      ? params.data?.external_timestamp
  ...
  valueFormatter: (params: any) => {
    ...
    if (useCreatedAtForCreatedAtColumn) {
      ...
      return dayjs.utc(params.value).tz('Asia/Kolkata').format('DD MMM YYYY hh:mm:ss A');
    }

    // VA -> already IST
    if (params.data?.alert_section === 'VA') {
      return dayjs(params.value).format('DD MMM YYYY hh:mm:ss A');
    }

    // Other sections -> UTC to IST
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex justify-between items-center mb-2 space-x-2">
        ...
        >
          <RefreshCw className={clsx("mr-2 h-4 w-4 transition-transform", {
