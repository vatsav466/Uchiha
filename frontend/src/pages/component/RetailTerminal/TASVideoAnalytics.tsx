...
const TASVideoAnalytics = () => {  
  ...
  // In handlePlantChange
  const handlePlantChange = (plant) => { 
    console.log('handlePlantChange called with:', plant);
    setLocationFilter(prev => {
      const newState = {
        ...prev,
        plant
      };
      console.log('New locationFilter state:', newState);
      return newState;
    });
    setCurrentPage(1);
  };
  ...
  const buildQueryString = () => {
    ...
    const baseQuery = "bu='TAS' AND alert_section='VA'";
  ...
  // For testing only
  useEffect(() => {
    if (locationFilter.zone === 'PARADEEP TERMINAL') { // Replace with actual zone value
      console.log('Setting PARADEEP plant ID manually');
      handlePlantChange('1630'); // Replace with actual SAP ID
    }
  }, [locationFilter.zone]);
  ...
  return (
    <div className="space-y-1 bg-white pt-3 rounded-lg shadow-md">
      ...
      <Card className="ml-2 w-[95%] md:w-[98%] lg:w-[99%] rounded-lg shadow-sm">
      ...
      {/* Media Modal (Handles both images and videos) */}
      <Dialog
...
export default TASVideoAnalytics;