...
export const TruckNumbersSection: React.FC<TruckNumbersSectionProps> = ({
  ...
  const truckData = linkedRowsData.length > 0 ? linkedRowsData : truckNoStrings.map((num: string) => ({ vehicle_number: num, truck_no: num }));
  ...
  // Extract truck information from the data; only include rows that have a real vehicle identifier
  const truckInfo = truckData
    .map((row: any, index: number) => {
      ...
      return {
        ...
        raw: row,
        hasRealId: !!(truckNumber || ttNumber || tripName || tripId),
      };
    })
  ...
  return (
    <SectionWrapper title={` Vehicle numbers (${truckInfo.length})`} className="mb-2 pt-1 pb-1">
