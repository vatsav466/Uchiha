...
export function TicketFormPage() {
  ...
  useEffect(() => {
    ...
    const fetchTicketForEdit = async () => {
      ...
      try {
        // Fetch the single ticket (do NOT call the "get all" list API)
        try {
          const res = await apiClient.get(`/api/ticketing/${routeTicketId}`);
          if (isMounted) setTicketToEdit(res.data as Ticket);
          return;
        } catch (directErr: any) {
          // Fallback: query the list endpoint but filtered to this ticket_id (limit 1)
          const listRes = await apiClient.get(`/api/ticketing`, {
            params: {
              skip: 0,
              limit: 1,
              q: `ticket_id='${routeTicketId}'`,
            },
          });
    ...
    if (isEditMode) {
      // Prefer navigation state; otherwise fetch by ticket_id from API
      if (ticketFromState) {
  ...
  const handleCancelForm = () => {
    ...
    if (parentTicketId && parentTicketId.trim() !== "") {
      navigate(`/settings/edit-ticketing/${encodeURIComponent(parentTicketId)}`);
    } else if (alertId) {
          ...
          (isEditMode && ticketToEdit && !isLoadingPage) ? (
            <>
              <TicketFormCore
                ...
                initialData={parentTicketId ? null : ticketToEdit}
