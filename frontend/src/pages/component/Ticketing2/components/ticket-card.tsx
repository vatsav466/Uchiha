...
const ticketStateColors: Record<Ticket["ticket_state"], string> = {
  ...
  "Returned By Occ": "bg-purple-100 text-purple-700 border-purple-300",
  "Reviewed By Occ": "bg-green-100 text-green-700 border-green-300",
  "Returned By OCC": "bg-purple-100 text-purple-700 border-purple-300",
  "Reviewed By OCC": "bg-green-100 text-green-700 border-green-300",
};
    ...
    <div className="mb-1.5 overflow-visible">
      ...
      {showStateInListView && (
        <div className="mb-2">
          ...
          >
            State: {getTicketStateDisplayLabel(ticket.ticket_state)}
          </Badge>
        </div>
      )}
...
>
  {(() => {
    ...
    // Resolved/Cancelled/ReviewedByOcc show "Close"; ON COMPLETED shows "Open" on card (still counted in header CLOSED)
    const isClosed = ["resolved", "cancelled", "reviewedbyocc"].some((s) => state === s);
    const isOpen = ["todo", "inprogress", "onhold", "reopen", "oncompleted", "open", "escalated", "updatedbyinitiator", "returnedbyocc"].some((s) => state === s);
