...
import { getTicketStateDisplayLabel } from "../ticket-form-constants";
...
const stateColors: Record<Ticket["ticket_state"], string> = {
    ...
    "Returned By Occ": "bg-purple-100 text-purple-700 border-purple-200",
    "Reviewed By Occ": "bg-green-100 text-green-700 border-green-200",
    "Returned By OCC": "bg-purple-100 text-purple-700 border-purple-200",
    "Reviewed By OCC": "bg-green-100 text-green-700 border-green-200",
};
...
export function TicketDetailsPage() {
    ...
    const handleMergeTicketClick = async (mergeTicketId: string) => {
        ...
        navigate(`/settings/edit-ticketing2/${mergeTicketId}`);
    };
    ...
    useEffect(() => {
        ...
        // Always fetch latest ticket details once per ticketId in edit page.
        if (fetchedTicketIdRef.current === ticketId) return;
        fetchedTicketIdRef.current = ticketId;
            ...
            <Card className="shadow-lg">
                <CardHeader className="border-b bg-gray-50 py-3">
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                            <Link
                                to={`/settings/add-edit-ticketing2?parent_id=${encodeURIComponent(String(ticket.ticket_id))}`}
                                state={{}}
                            >
                                <Button
                                    variant="outline"
                        ...
                        <div className="p-4 space-y-4">
                            <SectionPage title="Core Information">
                                ...
                                <div className="flex items-center gap-1.5 text-sm">
                                    ...
                                    >
                                        {getTicketStateDisplayLabel(ticket.ticket_state)}
                                    </Badge>
                                </div>
                                        ...
                                        <>
                                            {ticket.subtask_id.filter((id) => id && String(id).trim() !== "").map((subtaskId, index) => (
                                                <span key={`subtask-${subtaskId}-${index}`} className="flex items-center gap-1">
                                                    <button
                                                        type="button"
                                                        onClick={() => navigate(`/settings/edit-ticketing2/${subtaskId}`)}
                                        ...
                                        <SectionPage title="Core Information">
                                            ...
                                            <DetailItemPage
                                                ...
                                                value={
                                                    ...
                                                    >
                                                        {getTicketStateDisplayLabel(selectedMergeTicketDetails.ticket_state)}
                                                    </Badge>
                                                }
                                            />
                                            <DetailItemPage
