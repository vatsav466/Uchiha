...
import { Ticketing2Dashboard } from './Ticketing2Dashboard';
...
export const TicketDialogModal: React.FC<TicketDialogModalProps> = ({
  ...
  // Ticket selected from the dashboard view; rendered in the form (edit mode) inside this same modal.
      ...
      >
        ...
        <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
          {showDashboard ? (
            <Ticketing2Dashboard
              onCreateTicketClick={() => {
                // Open a fresh create form inside this modal instead of navigating away.
                setSelectedTicket(null);
                setShowDashboard(false);
              }}
              onTicketClick={(ticket) => {
                // Open the clicked ticket in the same edit/create form view inside this modal.
            ...
            <CreateTicketDialog
              // Remount the form when the ticket being viewed/edited changes (or when toggling between create/edit).
              key={`ticket-form-${selectedTicket?.id ?? selectedTicket?.ticket_id ?? 'create'}`}
              open={true}
              onOpenChange={(open) => {
              ...
              initialData={selectedTicket ?? initialData}
              onSubmitSuccess={() => {
                setSelectedTicket(null);
                setShowDashboard(true);
              }}
            ...
            <div className="flex flex-col items-start">
              <span className="text-sm font-semibold">
                {showDashboard
                  ? 'Tickets Dashboard'
                  : selectedTicket
                    ? `Edit Ticket${selectedTicket.ticket_id ? `: ${selectedTicket.ticket_id}` : ''}`
                    : 'Create Ticket'}
              </span>
