...
interface HostOverloadedTtsResponse {
  data: HostOverloadedTts[];
  total: number;
  count: number;
}
...
export const HostOverloadedTts: React.FC<TableProps> = ({
  zone,
  plant,
  dateFilter,
  bcuQuery,
  ...
  const [rowData, setRowData] = useState<HostOverloadedTts[]>([]);
  ...
  const fetchData = useCallback(async (page = currentPage, size = pageSize) => {
    ...
    try {
      ...
      const response = await apiClient.get<HostOverloadedTtsResponse>('/api/hostoverloadedtts', { params });
  ...
  const handleExcelExport = useCallback(async () => {
    ...
    try {
      ...
      worksheetData.push([`HOST OVERLOADED TTS REPORT - ${formattedDateForTitle} at ${formattedTimeForTitle}`]);
      ...
      if (bcuQuery && bcuQuery.trim() !== '') {
        appliedFilters.push(`BCU Filter: Applied`);
      }
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, 'Host Overloaded TTS');

      // Generate filename with detailed timestamp
      const fileTimestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, -5);
      const filename = `Host_Overloaded_TTS_Report_${fileTimestamp}.xlsx`;
...
export default HostOverloadedTts;