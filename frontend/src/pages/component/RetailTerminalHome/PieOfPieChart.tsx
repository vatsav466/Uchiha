...
const PieOfPieChart: React.FC = () => {
  ...
  fill: am5.color("#ffffff")
    ...
    const processData = (interlockAlerts) => {
        return interlockAlerts.map(alert => ({
          category: alert.interlock_name,
          value: (alert.High || 0) + (alert.Medium || 0) + (alert.Critical || 0),
          subData: [
            { category: `Critical (${alert.Critical || 0})`, value: alert.Critical || 0 },
            { category: `Medium (${alert.Medium || 0})`, value: alert.Medium || 0 },
            { category: `High (${alert.High || 0})`, value: alert.High || 0 }
          ]
        }));
      };
    ...
    const processedData = processData(chartData.interlock_alerts);
    ...
    function selectSlice(slice: any) {
        ...
        if (dataContext) {
          subSeries.data.setAll(dataContext.subData); // Dynamically update the sub-chart with subData
        }
