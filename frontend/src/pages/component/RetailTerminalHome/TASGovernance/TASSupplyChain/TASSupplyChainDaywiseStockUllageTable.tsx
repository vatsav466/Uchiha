...
export default function TASSupplyChainDaywiseStockUllageTable({
  rows,
  isLoading,
  selectedProduct,
}: Props) {
  const tableRows = useMemo(() => {
    ...
    rows.forEach((row) => {
      ...
      if (!previous) {
        grouped.set(day, { day, availableStock, ullage });
        return;
      }

      grouped.set(day, {
        day,
        availableStock: previous.availableStock + availableStock,
        ullage: previous.ullage + ullage,
      });
    });
      ...
      <table className="w-full min-w-[320px] border-collapse text-[11px]">
        <thead className="sticky top-0 z-[1] bg-slate-50">
          <tr>
            ...
            <th className="border-b border-slate-200 px-2 py-1.5 text-right font-semibold text-amber-600">
              Available Stock (KL)
            </th>
            <th className="border-b border-slate-200 px-2 py-1.5 text-right font-semibold text-cyan-600">
              Ullage (KL)
            </th>
          </tr>
        </thead>
