...
interface TtNumberDetailSheetProps {
  isOpen: boolean;
  onClose: (open: boolean) => void;
  clickedTtNumber: string | null;
  ttLineChartData: any[];
  ttLineChartLoading: boolean;
  ttLineChartError: string | null;
  ttModalData: any;
  ttModalLoading: boolean;
  ttModalError: string | null;
  ttModalSearch: string;
  setTtModalSearch: (val: string) => void;
  onDownload: () => void;
  dateBadgeLabel: string;
}

const TtNumberDetailSheet: React.FC<TtNumberDetailSheetProps> = ({
  isOpen,
  onClose,
  clickedTtNumber,
  ttLineChartData,
    ...
    <Sheet open={isOpen} onOpenChange={onClose}>
      ...
      <SheetContent className="w-[90vw] max-w-[90vw] h-screen p-4 flex flex-col overflow-y-auto [&>button]:hidden outline-none border-none">
        <SheetHeader className="pb-2">
          ...
          {ttModalData?.message && (
            <SheetDescription className="text-sm text-gray-600">
              DATE WISE CUMULATIVE RISK SCORE OF TT_NO: {clickedTtNumber}
            </SheetDescription>
          )}
        </SheetHeader>
        ...
        <div className="mb-1 bg-white rounded-lg border border-gray-200 shadow-sm p-0 overflow-visible">
          <RiskScoreLineChart
            data={ttLineChartData}
            loading={ttLineChartLoading}
            error={ttLineChartError}
          />
        </div>

        {/* Search & Download for Table */}
        {ttModalData?.data && Array.isArray(ttModalData.data) && ttModalData.data.length > 0 && (
          <div className="flex items-center gap-2 -mb-2 ">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="Search..."
                value={ttModalSearch}
                onChange={(e) => setTtModalSearch(e.target.value)}
        ...
        <div className="flex-1 min-h-0">
          {ttModalLoading ? (
            <div className="flex items-center justify-center py-8">
              ...
              <span className="ml-2 text-sm text-gray-600">Loading TT details...</span>
            </div>
          ) : ttModalError ? (
            <div className="p-3 bg-red-50 border border-red-200 rounded">
              <p className="text-red-800 text-sm font-medium">Error</p>
              <p className="text-red-600 text-xs mt-0.5">{ttModalError}</p>
            </div>
          ) : ttModalData?.data ? (
            <div className="h-full">
              {Array.isArray(ttModalData.data) && ttModalData.data.length > 0 ? (
                <div className="border border-gray-200 rounded overflow-hidden h-full flex flex-col">
                  <div className="bg-gray-50 px-4 py-2 border-b border-gray-200">
                    <p className="text-xs text-gray-600">
                      Total Records: <span className="font-semibold">{ttModalData.total_records || ttModalData.data.length}</span>
                    </p>
                  </div>
                  <div className="overflow-x-auto overflow-y-auto flex-1" style={{ maxHeight: 'calc(95vh - 180px)' }}>
                    {(() => {
                      const allTtKeys = Object.keys(ttModalData.data[0]);
                      const hideInTtSheet = ['tt_number', 'transporter_code', 'transporter_name', 'location_type'];
                      const priorityTtOrder = ['version_date', 'daterange', 'date_range', 'total_trips', 'risk_score'];
                      const priorityTtKeys = priorityTtOrder.filter(pk => allTtKeys.some(k => k.toLowerCase() === pk));
                      const actualPriorityTtKeys = priorityTtKeys.map(pk => allTtKeys.find(k => k.toLowerCase() === pk)!);
                      const remainingTtKeys = allTtKeys.filter(k => !priorityTtOrder.includes(k.toLowerCase()) && !hideInTtSheet.includes(k.toLowerCase()));
                      const visibleTtKeys = [...actualPriorityTtKeys, ...remainingTtKeys];
                      const versionDateKey = allTtKeys.find(k => k.toLowerCase() === 'version_date') ?? null;
                      ...
                      return (
                        <table className="min-w-full divide-y divide-gray-200 text-xs">
                          ...
                          <tbody className="bg-white divide-y divide-gray-200">
                            {ttModalData.data
                              .filter((row: any) => {
                                if (!ttModalSearch) return true;
                                return Object.values(row).some((val: any) =>
                                  String(val ?? '').toLowerCase().includes(ttModalSearch.toLowerCase())
                                );
                              })
                              .sort((a: any, b: any) => {
                              ...
                              .map((row: any, idx: number) => (
                                <tr key={idx} className="hover:bg-gray-50">
                                  {visibleTtKeys.map((colKey, cellIdx) => {
                ...
                <div className="p-3 bg-gray-50 border border-gray-200 rounded">
                  <p className="text-xs text-gray-600">No data found for this TT number</p>
                </div>
              )}
            </div>
          ) : (
...
export default TtNumberDetailSheet;
