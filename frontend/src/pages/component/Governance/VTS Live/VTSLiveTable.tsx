...
export const VTSLiveTable: React.FC<VTSLiveTableProps> = ({
    selectedBu ,
    selectedZone ,
    selectedPlant ,
    crossFilters = [
...
const getChartData = () => {
    ...
    const currentDrillKey = drillDownFilters.length > 0 
        ? drillDownFilters[drillDownFilters.length - 1].key 
        : null;

    // Define which column should be used as 'name' at each level
    let nameColumn: string | null = null;

    if (currentDrillKey === "transporter_name") {
        // Next level down → tt_number
        nameColumn = "tt_number";
    } else if (currentDrillKey === "location_name") {
        nameColumn = "transporter_name";
    } else if (currentDrillKey === "zone") {
        nameColumn = "location_name";
    } else {
        nameColumn = "zone"; // Default top-level
    }

    // Fallback if column doesn’t exist
    if (!columns.includes(nameColumn)) {
        nameColumn = columns.find(col =>
            col.toLowerCase().includes("name") ||
            col === "zone" ||
            col === "location_name" ||
            col === "transporter_name" ||
    ...
    // Get numeric columns for chart values
    const numericColumns = columns.filter(col => {
...
{!drillDownFilters.some(f => f.key === "tt_number") && (
    <div className="rounded-xl shadow-lg overflow-hidden border border-gray-200 bg-white p-6">
        ...
        {(loading || drillDownLoading) ? (
            ...
            <AmChartsBarChart
                data={chartData}
                chartId={`vtslive-chart-${drillDownFilters.length}`}
            />
