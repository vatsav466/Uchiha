...
const ExapandedGDRejections = () => {
    ...
    const resetFilters = async () => {
        ...
        try {
            // Fetch initial filter options with no filters first
            const response = await apiClient.post('/api/charts/generate_vis_data', {
                    filters: [],
                    action: "operations_dropdown",
                    drill_state: "",
            ...
            // Reset filters only after fetching filter data
            const resetValues = Object.keys(result).reduce((acc, key) => {
                acc[key] = result[key].includes("NULL") ? "NULL" : "";  // Reset only with the fetched data
                return acc;
            }, {} as Record<string, string>);
    
            setSelectedFilters(resetValues);  // Reset filter values
    
            // Reset active filters (and only after above states are updated)
            setActiveFilters([{
                key: '"rejection_type"',
                cond: "equals",
                value: "GD"
            ...
            // Fetch data with reset filters (after state reset is completed)
            await fetchData([{
                key: '"rejection_type"',
                cond: "equals",
                value: "GD"
    ...
    useEffect(() => {
        ...
        const scrollbarX = am5.Scrollbar.new(root, {
            ...
            minHeight: 10,
            start: 0,
            // end: chartData.length === 7 ? 1 : 5 / chartData.length
        });
        chart.set("scrollbarX", scrollbarX);
        chart.bottomAxesContainer.children.push(scrollbarX);
...
export default ExapandedGDRejections;