...
interface LPGoperationsproductionzoneChartProps {
  activeFilters: Array<{
    key: string
    cond: string
    value: string
...
// { key: "carousel_type", label: "carousel type" },
...
const LPGoperationsproductionzoneChart = ( {
  activeFilters,
  crossFilters,
  onResetFilters,
  selectedFromDate,
  selectedToDate,
}:LPGoperationsproductionzoneChartProps) => {
  ...
  const resetFilters = async () => {
    ...
    try {
      ...
      const response = await apiClient.post('/api/charts/generate_vis_data', {
        filters: [],
        cross_filters: [],
        action: "lpg_operations_production_zone",
        drill_state: ""
      });
  ...
  const transformData = (data: ApiResponse['data'], currentLevel: string = drilldownState.level): ChartDataItem[] => {
    if (!Array.isArray(data) && currentLevel === 'zone') {
      return Object.keys(data.zone).map(key => ({
        ...
        value: parseFloat(data.Productions[key].toString()),
        drillDown: true
      }));
    }
    ...
    // Handle the plant level data
    if (currentLevel === 'plant' && Array.isArray(data)) {
      return data.map(item => ({
        zone: item.name, // Use the 'name' field for display
        name: item.name, // Keep name for consistency
        value: parseFloat(item.Productions.toString()),
        drillDown: false // Plant is the lowest level, no more drilling
      }));
    }
      
    // This is the original grouping logic for other cases
    const groupByField = {
      zone: 'zone',
      plant: 'plant',
    ...
    data.forEach(item => {
      ...
      if (typeof key === 'string') {
        ...
        groupedData[key] += parseFloat(item.Productions.toString());
      }
    });
  ...
  const fetchData = async (filters: DrilldownState['filters'], crossFilters: any[], level?: string) => {
    try {
      ...
      const payload = {
        ...
        action: "lpg_operations_production_zone", // Changed from "lpg_operations_filled_cylinder"
        drill_state: ""
      };
  ...
  // Combined initial data fetch to prevent duplicate calls
  useEffect(() => {
    // Prevent duplicate initialization on strict mode or hot reload
  ...
  // Fetch data when drilldown filters change
  useEffect(() => {
    if (!hasInitialized.current) return;
  ...
  const title = `Production (TMT) (${selectedFromDate && selectedToDate ? `${selectedFromDate} to ${selectedToDate}` : formattedDate})`;
...
export default LPGoperationsproductionzoneChart;
