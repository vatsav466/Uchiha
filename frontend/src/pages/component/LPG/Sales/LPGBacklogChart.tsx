...
interface ApiResponse {
  status: boolean;
  message: string;
  overall_number: number;  // Add this to the interface
  data: {
    ZOName: { [key: string]: string };
    "Backlog": { [key: string]: number };
  } | Array<{
    ZOName: string;
    ROName: string;
    SAName: string;
...
const LPGBacklogChart = () => {
  ...
  const transformData = (apiResponse: ApiResponse): ChartDataItem[] => {
    ...
    // Update the total backlog from the API's overall_number
    setTotalBacklog(apiResponse.overall_number);
    ...
    if (Array.isArray(apiResponse.data)) {
      ...
      apiResponse.data.forEach(item => {
        ...
        if (typeof key === 'string') {
          ...
          const value = parseFloat(item.Backlog?.toString() || '0');
  ...
  const fetchData = async (filters: DrilldownState['filters'], crossFilters: any[]) => {
    try {
      ...
      if (result.status) {
        ...
        // Set total backlog from overall_number
        if (result.overall_number !== undefined) {
          setTotalBacklog(result.overall_number);
        }
...
let series = chart.series.push(
    am5xy.ColumnSeries.new(root, {
      ...
      categoryXField: "zone",
      tooltip: am5.Tooltip.new(root, {
        labelText: "[bold fontSize: 8px]{categoryX}[/]\n[fontSize: 8px]Total Backlog: [fontSize: 8px bold]{valueY}[/]",
    ...
    <div className="relative">
      <div className={`transition-all duration-300 ${isExpanded ? 'fixed inset-4 z-50' : ''}`}>
        ...
        <CardTitle className="text-xs font-bold text-gray-800 whitespace-nowrap">
            ...
            Total Backlog (In Days) ({getYesterdayDate()})
            </CardTitle>
