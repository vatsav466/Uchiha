...
// In the transformData function, modify to use the overal_number:
const transformData = (apiResponse: ApiResponse): ChartDataItem[] => {
  if (!apiResponse.status || !apiResponse.data) {
    setTotalSales(0); // Set to 0 if no data
    return [];
  }
  // Update the total sales with the overal_number from the response
  setTotalSales(apiResponse.overal_number ?? 0);  // Use nullish coalescing to default to 0

  if (drilldownState.level === 'zone' && !Array.isArray(apiResponse.data)) {
    // Handle ZOName level data
    const data = apiResponse.data;
    
    if (data.ZOName[0]) {
      return [{
        zone: data.ZOName[0],
        value: data.pcc_prorated,
        drillDown: true
      }];
    }
    return [];
    ...
    apiResponse.data
      ...
      .forEach(item => {
        ...
        if (typeof key === 'string') {
          ...
          const value = parseFloat(item.pcc_prorated?.toString() || '0');
...
const resetFilters = async () => {
  ...
  const handleBackClick = async () => {
    if (drilldownState.filters.length > 0) {
      ...
      const levels: DrilldownState['level'][] = ['zone', 'region', 'salesarea', 'distributor'];
...
let series = chart.series.push(
    am5xy.ColumnSeries.new(root, {
      ...
      categoryXField: "zone",
      tooltip: am5.Tooltip.new(root, {
        labelText: "[bold fontSize: 8px]{categoryX}[/]\n[fontSize: 8px]Total PCC: [fontSize: 8px bold]{valueY}[/]",
  ...
  series.columns.template.adapters.add("fill", (fill, target) => {
    const colors = [
        am5.color(0x7c9d5a), // A fresh green
        am5.color(0xd97531), // A rich burnt orange
        am5.color(0x6a4c4f), // A deep mauve
        am5.color(0x3c8e7b), // A cool teal
        am5.color(0xc4b92d), // A mustard yellow
        am5.color(0x703f8c), // A deep violet
        am5.color(0xf5b93e), // A golden amber
        am5.color(0x49b6ab), // A soft seafoam green
        am5.color(0x6f77c1), // A lavender blue
        am5.color(0x4a5f4e), // A dark sage green
      ];
...
export default LPGBacklogChart;
