...
const OverallSafetyCheckPending = () => {
  ...
  const fetchData = async (crossFilters = []) => {
    try {
      ...
      const payload = {
        ...
        action: "lpg_cdcms_safety_check_pending",
        drill_state: "",
      };
  ...
  useEffect(() => {
    ...
    pieLegend.labels.template.setAll({
      fontSize: 10,
      text: "{name}" // Show only category name
    });
    ...
    <Card >
      ...
      {drillHistory.length > 0 && (
        ...
        <div className="flex h-full gap-0 relative">
          ...
          <div ref={barChartDivRef} className="p-0 w-[80%] h-300px"  />
        </div>
      </CardContent>
    </Card>
  );
...
export default OverallSafetyCheckPending;
