...
interface OldDrillDownChartProps {
  data: OldDrillDownResponse;
  loading: boolean;
  error: string | null;
  title?: string;
}
...
/** OLD: one row per top-level group with name "OLD 1" / "OLD 2", sub-keys become grouped bars. X-axis shows only "OLD 1" or "OLD 1" & "OLD 2". */
function normalizeOldHandledSortoutByGroup(data: Record<string, unknown>): {
  totalSortout: number;
  ...
  for (const g of topKeys) {
    ...
    const row: ChartDataRow = { name: `OLD ${g}` };
...
/** Normalize API response into chart data and summary. Handles nested { "1": { "1": { handled, sortout }, ... }, "2": { ... } } → x-axis "OLD 1" / "OLD 2" with grouped bars or legacy shapes */
function normalizeDrillDownResponse(data: OldDrillDownResponse): {
  totalSortout: number;
  ...
  const byGroup = normalizeOldHandledSortoutByGroup(data as Record<string, unknown>);
  if (byGroup) {
    return {
  ...
  const stackKeys = ['Leak', 'No Connect', 'Time Out', 'Others'];
  const chartData: ChartDataRow[] = [];
...
export const OldDrillDownChart: React.FC<OldDrillDownChartProps> = ({ data, loading, error, title = 'OLD Rejections Drill Down' }) => {
  const { totalSortout, breakdown, chartData, stackKeys, barKeys } = useMemo(() => normalizeDrillDownResponse(data), [data]);
...
export default OldDrillDownChart;
