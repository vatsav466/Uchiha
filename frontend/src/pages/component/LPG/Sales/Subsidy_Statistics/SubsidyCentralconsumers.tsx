...
// Function to format raw number values for tooltips (original value)
...
const SalesComparisonChart = ({ globalActiveFilters, globalCrossFilters }: SalesComparisonChartProps) => {
  ...
  const fetchChartData = async () => {
    ...
    try {
      ...
      const response = await apiClient.post("/api/charts/generate_vis_data", {
        ...
        action: "lpg_cdcms_subsidy_central_consumers",
        drill_state: "",
      })
      ...
      if (data.status) {
        const transformedData = Array.isArray(data.data)
          ? data.data.map((item: SalesComparisonDataItem) => {
              // Calculate totals
              if (item.PMUY) {
                pmuyTotal += Number.parseFloat(item.PMUY.toString())
              }
              if (item.NPMUY) {
                npmuyTotal += Number.parseFloat(item.NPMUY.toString())
              }
              return {
                ...item,
  ...
  const resetFilters = async () => {
    ...
    try {
      ...
      // Reset cross filters while preserving global filters
      const updatedCrossFilters = [...globalCrossFilters].filter((filter) => !filter.key.includes("DATE"))
      setCrossFilters(updatedCrossFilters)

      // Fetch initial filter options with only global filters
      const response = await apiClient.post("/api/charts/generate_vis_data", {
      ...
      if (result) {
        setFilterData(result)
        // Explicitly fetch new chart data after filter reset
  ...
  useEffect(() => {
    fetchChartData()
  }, [globalActiveFilters, globalCrossFilters, drillLevel])
  ...
  const LoadingOverlay = () => (
    <div className="absolute inset-0 bg-white/80 flex items-center justify-center z-10">
      <div className="flex flex-col items-center gap-2">
        ...
        <span className="text-xs text-gray-600">Loading {getDrillLevelField(drillLevel)} data...</span>
      </div>
    </div>
  )
  const toggleExpand = () => {
        ...
        <CardHeader className="pb-0 p-1">
            ...
            >
              <div className="flex items-center gap-4 justify-between">
                <CardTitle className="text-xs font-bold text-gray-800 whitespace-nowrap">
                  Central Subsidy Consumers Count
                </CardTitle>
          ...
          {/* <div className="flex gap-4 text-xs font-semibold">
            <span className="text-black-600">
              Total PMUY:
              <span className="text-blue-600 ml-1">{formatTotal(totalSales.current)}</span>
            </span>
            <span className="text-black-600">
              Total NPMUY:
              <span className="text-blue-600 ml-1">{formatTotal(totalSales.previous)}</span>
            </span>
          </div> */}
        </CardHeader>

        <CardContent className={`p-0 pb-0 relative ${isExpanded ? "h-[calc(100vh-8rem)]" : "h-[500px]"} pt-0`}>
