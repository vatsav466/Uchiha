...
interface DrilldownState {
    level: 'paymenterrorname' | 'zone' | 'region' | 'salesarea' | 'distributor';
    filters: Array<{
        key: string;
        cond: string;
        value: string;
...
interface ApiResponse {
    ...
    message: string;
    data: {
        PaymentErrorName?: { [key: string]: string };
...
const DrillStateIndicator = ({ level }: { level: string }) => {
    ...
    const states = ['PaymentErrorName', 'Zone', 'Region', 'SalesArea', 'Distributor'];
    
    // Normalize the level name to match the states array
    const normalizedLevel = level.toLowerCase();
    const currentIndex = states.findIndex(state => state.toLowerCase() === normalizedLevel);
    
    // Create display names with proper capitalization
    const displayNames = ['Payment Error Name', 'Zone', 'Region', 'SalesArea', 'Distributor'];
...
const BigNumberCard = ({ title, consumers, refills, onClick,totalRefills   }) => {
    ...
    return (
      <Card
        className="h-22 cursor-pointer hover:shadow-lg transition-shadow bg-[#424771]"
        onClick={onClick}
      >
        <div className="h-full flex flex-col">
          ...
          <div className="flex-1 flex items-center justify-between px-2">
            <div className="flex flex-col">
              <span className="text-[12px] text-white">Consumers</span>
              <p className="text-[9px] font-bold text-white">
                {consumers?.toLocaleString()}
              </p>
            </div>
            <div className="flex flex-col items-end">
              <span className="text-[13px] text-white">Refills</span>
              <p className="text-[9px] font-bold text-white">
...
const SubsidyFailureStats = () => {
  ...
  const [selectedCardData, setSelectedCardData] = useState(null);
    ...
    const handleBackClick = async () => {
        if (drilldownState.filters.length > 0) {
            ...
            const levels: DrilldownState['level'][] = ['paymenterrorname', 'zone', 'region', 'salesarea', 'distributor'];
            const newLevel = levels[newFilters.length] || 'paymenterrorname';

            // If going back to initial level with no filters, show cards
            if (newLevel === 'paymenterrorname' && newFilters.length === 0) {
                setShowCards(true);
            }
    ...
    const transformData = (data: ApiResponse['data']) => {
        if (drilldownState.level === 'paymenterrorname' && 'PaymentErrorName' in data) {
            // Transform data for both cards and chart when at payment error level
            const keys = Object.keys(data.PaymentErrorName);
            const transformedData = keys.map(key => ({
                category: data.PaymentErrorName[key],
            ...
            // Update card data when at payment error level
            ...
            const groupedData: { [key: string]: { consumers: number; refills: number } } = {};
 ...
 const getDrillLevelField = (level: number) => {
    const fields = ['PaymentErrorName', 'ZOName', 'ROName', 'SAName', 'DistributorName'];
    return fields[level] || fields[0];
};
const fetchData = async (drillFilters: LocationFilter[], crossFilters: LocationFilter[]) => {
    try {
        setIsTransitioning(true);
        ...
        const response = await apiClient.post('/api/charts/generate_vis_data', {
                ...
                action: "lpg_cdcms_subsidy_failure_stats",
                drill_state: drilldownState.level
            });
        ...
        if (result.status && result.data) {
            ...
            // Show cards only at initial payment error level with no filters
            if (drilldownState.level === 'paymenterrorname' && drillFilters.length === 0) {
...
};    const handleCardClick = (paymentErrorName) => {
        setSelectedCardData(paymentErrorName);
        setShowCards(false);
  //Update set showCards in the   
        // Update drilldown state to zone level with filter for the selected payment error
        setDrilldownState({
            level: "zone",
            filters: [
                {
                    key: '"PaymentErrorName"',
                    cond: "equals",
                    value: paymentErrorName,
                },
            ],
        });
    
        // Add to drill history
        setDrillHistory([paymentErrorName]);
    ...
    useEffect(() => {
        ...
        xAxis.get("renderer").labels.template.setAll({
          ...
          const getyAxisLabel = () => {
            const labelMap = {
              'paymenterrorname': 'Payment Error Names',
              'zone': 'Zones',
              'region': 'Regions',
        ...
        const consumersSeries = chart.series.push(
            am5xy.ColumnSeries.new(root, {
                ...
                tooltip: am5.Tooltip.new(root, {
                    labelText: "[fontSize: 8px bold]{category}[/],[fontSize: 8px bold]Consumers: [fontSize: 8px bold]{valueX}[/]"
                })
            })
        );
        ...
        const refillsSeries = chart.series.push(
            am5xy.ColumnSeries.new(root, {
                ...
                categoryYField: "category",
                tooltip: am5.Tooltip.new(root, {
                    labelText: "[fontSize: 8px bold] {category}[/],[fontSize: 8px bold]Refills: [fontSize: 8px bold]{valueX}[/]"
                })
            })
        );
        ...
        [consumersSeries, refillsSeries].forEach((series, index) => {
            series.columns.template.setAll({
                ...
                cornerRadiusBR: 5,
                cornerRadiusTR: 5,
                fill: index === 0 ? am5.color(0xc05997) : am5.color(0x124964),
        ...
        const scrollbarY = am5.Scrollbar.new(root, {
            ...
            minHeight: 50, // Minimum height to ensure grabbability
            height: am5.percent(90), // Take up most of the available height
            animationDuration: 300, // Faster animation
            animationEasing: am5.ease.out(am5.ease.cubic), // Smoother easing
        });
    ...
    const renderCards = () => {
        ...
        // Split into two rows
        const firstRowCards = sortedCards.slice(0, 4);
        const secondRowCards = sortedCards.slice(4, 8);

        return (
            <div className="grid gap-2 p-2">
                <div className="grid grid-cols-4 gap-2">
                    {firstRowCards.map(({ paymentErrorName, consumers, refills, totalRefills }) => (
                        <BigNumberCard
                            key={paymentErrorName}
                            title={paymentErrorName}
                            consumers={consumers}
                            refills={refills}
                            totalRefills={totalRefills}
                            onClick={() => handleCardClick(paymentErrorName)}
                        />
                    ))}
                </div>
                <div className="grid grid-cols-4 gap-2">
                    {secondRowCards.map(({ paymentErrorName, consumers, refills, totalRefills }) => (
                        <BigNumberCard
                            key={paymentErrorName}
                            title={paymentErrorName}
                            consumers={consumers}
...
export default SubsidyFailureStats;