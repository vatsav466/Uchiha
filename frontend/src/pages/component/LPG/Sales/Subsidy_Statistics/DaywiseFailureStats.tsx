...
const SubsidyFailureChart: React.FC = () => {
  ...
  const fetchData = async (appliedFilters = []) => {
    ...
    try {
      const payload = {
        ...
        action: "lpg_cdcms_daywise_subsidy_failure_statistics",
  ...
  const aggregateDataByErrorType = (data: any[] | null): AggregatedData | null => {
    ...
    if (viewMode === "financial_year") {
      ...
      data.forEach(item => {
        ...
        const errorName = item.PaymentErrorName ? item.PaymentErrorName.trim() : "Unknown";
  ...
  useEffect(() => {
    ...
    const initChart = async () => {
      ...
      aggregatedData.seriesData
      .filter((seriesItem) => seriesItem.name && seriesItem.name !== "undefined")
      .forEach((seriesItem, index) => {
        // Create series without tooltip
        const series = chart.series.push(
          am5xy.LineSeries.new(root, {
            name: seriesItem.name === "undefined" ? "Unknown" : seriesItem.name,
            xAxis: xAxis,
            yAxis: yAxis,
            valueYField: "value",
            categoryXField: "date",
        ...
        // Add bullets with individual tooltips
        series.bullets.push((root, series, dataItem) => {
          const container = am5.Container.new(root, {});
          const valueY = dataItem.get("valueY");
    
          // Create the circle with its own tooltip
          ...
          circle.adapters.add("tooltipText", function() {
            return "[fontSize:10px bold]" + seriesItem.name + "[/]: [fontSize:10px bold]" + valueY + " [fontSize:10px bold]Refills," + dataItem.get("categoryX");
          });
      ...
      >
          ...
          <div className="flex flex-col gap-2">
            <div className="flex items-center justify-between">
              <CardTitle className="text-xs font-bold text-gray-800">
                Day Wise Subsidy Failure Statistics
              </CardTitle>
        ...
        <CardContent
          className={`p-0 relative ${
            isExpanded ? "h-[calc(100vh-8rem)]" : "h-[465px]"
          }`}
        >
          {isTransitioning && (
            <div
          ...
          <div
            ...
            style={{
              width: "100%",
              height: isExpanded ? "calc(100vh - 8rem)" : "450px",
            }}
          />
        </CardContent>
      </Card>
...
export default SubsidyFailureChart;