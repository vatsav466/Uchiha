...
const SubsidyStateconsumer =({ globalActiveFilters, globalCrossFilters }: SalesComparisonChartProps) => {
  ...
  const fetchChartData = async () => {
    ...
    try {
      ...
      const response = await apiClient.post("/api/charts/generate_vis_data", {
        ...
        action: "lpg_cdcms_subsidy_state_consumers",
        drill_state: "",
      });
  ...
  const resetFilters = async () => {
    ...
    try {
      ...
      const updatedCrossFilters = [...globalCrossFilters].filter(
        filter => !filter.key.includes('DATE')
      );
...
export default SubsidyStateconsumer;
