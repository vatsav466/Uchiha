...
const SubsidyStackTransaction = () => {
  ...
  // Get X-axis label based on drill level
  const getXAxisLabel = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await apiClient.post("/api/charts/generate_vis_data", {
          filters: [...drillState.filters, ...activeFilters].filter(
            (f) => f.value && f.value !== "placeholder"
          ),
          cross_filters: crossFilters,
          action: "lpg_cdcms_subsidy_state_transaction_stacked_state",
          drill_state: "",
        });
  ...
  useEffect(() => {
    ...
    const maxTransactionCount = Math.max(...chartData.map(item => item.transaction_count || 0));
    const yAxisMax = Math.ceil(maxTransactionCount * 1.2);
    ...
    xAxis.data.setAll(
      uniqueCategories.map(category => ({ [categoryField]: category }))
    );
    ...
    legend.markerRectangles.template.states.create("down", {});
     ...
     const customColors = [
      "#74C0FC", "#5C7CFA", "#9775FA", "#63E6BE", "#51CF66",
      "#00E5FF", "#6A0572", "#3D348B", "#218380", "#36D399"
    ]
    ...
    const processedData = uniqueCategories.map((category) => {
      ...
      uniqueStates.forEach((state) => {
        const items = chartData.filter(
          (item) => item[categoryField] === category && item.StateCode === state
        );

        const value = items.reduce((sum, item) => sum + (item.transaction_count || 0), 0);
        result[state] = value;
      });
  ...
  return (
    <>
      ...
      >
        ...
        <CardContent className={`p-0 relative ${isExpanded ? "h-[calc(100vh-8rem)]" : drillHistory.length == 0 ? "h-[350px]" : "h-[370px]"} pt-0`}>
...
export default SubsidyStackTransaction;