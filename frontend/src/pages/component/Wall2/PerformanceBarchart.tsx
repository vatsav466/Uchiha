  ...
  if (drillLevel === 0 && !isArray) {
    if (mode === "month" || mode === "ytd" || mode === "date") {
      if(responseData?.month_name) {
        ...
        return Object.keys(responseData.cumulative).map((key) => {
          const data: ChartData = { name: responseData.cumulative[key] };
          if (activeStates.A)
            data.ACTUAL_TMT_SALES = responseData.ACTUAL_TMT_SALES[key];
          if (activeStates.T)
            data.TARGET_TMT_SALES = responseData.TARGET_TMT_SALES
...
const SalesPerformanceDNC: React.FC = () => {
  ...
  const handleBackClick = useCallback(() => {
    ...
    if (drillLevel > 0) {
      ...
      // Find the highest active drill level
      const lastActiveDrill =
        activeDrills.length > 0 ? activeDrills[activeDrills.length - 1] : null;
      // const lastActiveCount = activeDrills.length > 0 ? activeDrills[activeDrills.length - 1] : null;
      const lastActiveCount =
        activeDrills.length > 1 ? activeDrills.slice(-2, -1)[0] : null;
      const drillcount = lastActiveDrill ? lastActiveCount?.drillLevel : 0;
      setDrillLevel(drillcount);

      if (activeDrills.length > 1) {
        const updatedList = drilldownList.map((drill, index) => {
      ...
      console.log("drillcount", drillcount);
  ...
  const DrillIndicator: React.FC<DrillStateIndicatorProps> = ({
    ...
    const lastActiveDrill = [...updatedDrilldownList]
      .reverse()
      .find((d) => d.isActive);
    const drillcount = lastActiveDrill ? lastActiveDrill.drillLevel : 0;

    console.log("updatedDrilldownList", updatedDrilldownList);
            ...
            <div className="flex">
              ...
              {mode === "month" && (
                <CustomMultiSelect
                  ...
                  className="w-auto min-h-4 p-1 bg-white hover:bg-white text-xs text-black shadow-none border-gray-300 ml-2"
                />
              )}
            </div>
            {/* All Buttons Group */}
              ...
              <div className="flex gap-1">
                ...
                {Object.keys(categoryData).map((key) => (
                  <Tooltip key={key}>
                    ...
                    <TooltipContent>
                      <p>
                        {categoryData[key as keyof typeof categoryData].name}
                      </p>
                    </TooltipContent>
                  </Tooltip>
                ))}
...
export default SalesPerformanceDNC;
