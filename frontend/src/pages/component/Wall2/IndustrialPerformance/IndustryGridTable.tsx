...
/** Full FY month order (April → March). Shown in the month dropdown. */
...
const IndustryPerformanceGrid = () => {
  ...
  const fetchData = useCallback(async (month, isCToggled?) => {
    setIsLoading(true);
    setError(null);
    try {
      // Create a base filters array with common filters
      const baseFilters = [
  ...
  useEffect(() => {
    if (selectedMonth) {
      fetchData(selectedMonth, isCToggled);
    }
  }, [selectedMonth, isCToggled, fetchData, dataRefreshKey]);


  const handleToggle = (isToggled) => {
    setIsCToggled(isToggled);
    fetchData(selectedMonth, isToggled);
  };

  const handleMonthChange = (month) => {
    setSelectedMonth(month);
    // fetchData(month, isCToggled);
  };
      ...
      <Card className="overflow-hidden relative">
        <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-x-3 gap-y-2 border-b p-2 py-1.5">
          <span className="text-sm font-bold leading-tight min-w-0 flex-1">
            Industry Performance for Marketing ({getCurrentFiscalYearLabel()})
          </span>
...
export default IndustryPerformanceGrid;