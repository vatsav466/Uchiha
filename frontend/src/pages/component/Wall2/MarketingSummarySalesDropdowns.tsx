  ...
  return (
    ...
    >
      ...
      <SelectContent onCloseAutoFocus={(e) => e.preventDefault()}>
        ...
        <SelectItem value="All" className="h-7 py-0 text-xs">All</SelectItem>
...
const MultiSelect: React.FC<MultiSelectProps> = ({
  ...
  const isAllSelected = filteredOptions.length > 0 && filteredOptions.every(option => value.includes(option))
...
export const MarketingSummarySalesDropdowns: React.FC<SalesDropdownsProps> = ({
  selectedYear,
  selectedSBU,
  selectedZone,
  selectedRegion,
  ...
  const handleChange = (key: string, value: string | string[]) => {
    ...
    switch (key) {
      case "year":
        handleYearChange?.(key as string, value as string)
        break
      case "sbu":
        handleSBUChange?.(key as string, value as string)
        break
      case "zone":
        handleZoneChange?.(key as string, value as string)
        break
      case "region":
        handleRegionChange?.(key as string, value as string)
        break
      case "salesArea":
        handleSalesAreaChange?.(key as string, value as string)
        break
      case "product":
        handleProductNameChange?.(key as string, value as string[])
        break
    }
  }
  ...
  return (
    <div className="flex text-xs flex-wrap gap-2 items-center">
      ...
      <MultiSelect
        ...
        onValueChange={(value) => handleProductNameChange?.("product", value)}
