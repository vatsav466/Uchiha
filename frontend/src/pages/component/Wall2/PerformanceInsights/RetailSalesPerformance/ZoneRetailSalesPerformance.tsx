...
const ZoneRetailSalesPerformance: React.FC = () => {
  ...
  const [sbuChartData, setSbuChartData] = useState([]);
  ...
  */
  ...
  const loadChartData = useCallback(async (drillState) => {
    setIsLoading(true);
    try {
      const retailFilter = { key: '"SBU_Name"', cond: "equals", value: "Retail" };
      ...
      if (response.status && response.data) {
        const transformedData = transformChartData(response.data?.data, "month", 0, activeStates);
        ...
        >
          ...
          {Object.entries(categoryData).map(([key, { color, name }]) => (
            activeStates[key] && (
              <Bar
                key={key}
                dataKey={key === 'A' ? 'actual' : key === 'H' ? 'historical' : 'target'}
                name={name}
                fill={color}
                maxBarSize={30}
              >
  ...
  return (
    <>
              ...
              {/* Chart Content */}
                <div className="w-[60%]">
                <div className="flex flex-col gap-4"> {/* Added container for charts */}
                  ...
                  <div className="h-[450px] w-full">
                    ...
                    <div className="h-[450px] w-full"> {/* Adjusted height */}
...
export default ZoneRetailSalesPerformance;
