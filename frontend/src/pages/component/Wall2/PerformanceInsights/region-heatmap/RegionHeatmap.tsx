...
interface Props {
  ...
  showTarget?: boolean;
  onCellClick?: (data: { region: string; month: string; value: number; mode: 'avsh' | 'avst' }) => void;
}
...
const RegionHeatmap: React.FC<Props> = ({
  data,
  xaxisData,
  growthDetailsAvH = [],
  growthDetailsAvT = [],
  ...
  const rows = useMemo(() => {
    return data.map((region) => {
      const cells = months.map((month) => {
        ...
        const actual  = Number(region[`${prefix}_actual`])  || 0;
        const history = Number(region[`${prefix}_history`]) || 0;
        const target  = Number(region[`${prefix}_target`])  || 0;
        return {
          month,
      ...
      return { region: region.Region_Name, cells };
    });
  }, [data, months]);
    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="w-full rounded-xl border border-gray-200 shadow-sm bg-white overflow-auto" style={{ maxHeight: 460 }}>
        <table className="w-full border-collapse text-[10px]" style={{ tableLayout: 'auto' }}>
          ...
          <tbody>
            {rows.map((row, ri) => (
              <tr key={row.region} className={ri % 2 === 0 ? 'bg-white' : 'bg-slate-50/40'}>
                  ...
                  <React.Fragment key={month}>
                    {showH && (
                      <td className="p-0.5 border-l border-gray-100">
                        <VCell
                          value={avh}
                          onClick={() => onCellClick?.({ region: row.region, month, value: avh, mode: 'avsh' })}
                        />
                      </td>
                    )}
                    {showT && (
                      <td className="p-0.5 border-l border-gray-50">
                        <VCell
                          value={avt}
                          onClick={() => onCellClick?.({ region: row.region, month, value: avt, mode: 'avst' })}
                        />
                      </td>
                    )}
                  </React.Fragment>
