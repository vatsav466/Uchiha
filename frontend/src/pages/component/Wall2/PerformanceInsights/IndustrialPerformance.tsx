  ...
  if (drillLevel === 0 && !isArray) {
    if (mode === "month" || mode === "ytd" || mode === "date") {
      return Object.keys(responseData.Company_Name).map((key) => {
        const data: ChartData = { name: responseData.Company_Name[key] };
        if (activeStates.A)
...
const IndustrialPerformanceInsights: React.FC = () => {
  ...
  const [selectedProductName, setSelectedProductName] = useState("(Select All");
  ...
  */
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetchChartData({
        filters: originalFilter, // !isDrillDown ? reorderedData : [], // Pass `filters` only if `isDrillDown` is true
  ...
  useEffect(() => {
    ...
    const initializeFilters = async () => {
      const years = await loadDistinctValues(
        mode === "year" ? ["FISCAL_YEAR"] : ["Company_Name"]
      );
      if (mode === "year") {
  ...
  const YTDBox = async () => {
    ...
    try {
      const response = await fetchChartData({
        filters: filter,
        cross_filters: [{ key: '"Company_Name"', cond: "equals", value: "" }],
  ...
  const handleBackClick = useCallback(() => {
    ...
    if (drillLevel > 0) {
      ...
      const nonPerspectiveFilters = filters.filter(
  ...
  const handleModeChange = useCallback(
    (newMode) => {
      ...
      if (newMode === "month" && mode === "month" && !filters.length) {
        return;
      }
  ...
  const handleMonthSelect = (monthValues) => {
    ...
    setFilters((prev) => {
      ...
      if (monthFilterIndex >= 0) {
        const newFilters = [...prev];
        newFilters[monthFilterIndex] = companyFilter;
        return newFilters;
      }
      return [...prev, companyFilter];
    });
  };
  ...
  const handleMonthChange = async (event: SelectChangeEvent<string>) => {
    ...
    setFilters([
      ...
      { key: '"FISCAL_YEAR"', cond: "equals", value: "FY 2024-2025" },
      { key: '"Company_Name"', cond: "equals", value },
    ]);
    setDrillLevel(1);
  ...
  const handleCompanyChange = async (event: SelectChangeEvent<string>) => {
    if (isDrillDown) {
      toast.warning(
        "You are switching from Drill to Dropdown filtering. This will reset your current filters."
      );
      resetFilters();
      setFilters([
        {
    ...
    setFilters([
      ...perspectiveFilters,
      {
        key: '"Company_Name"',
        cond: "equals",
        value: value === "PetChem" ? "PETROCHEMICALS SBU" : value,
      },
    ]);
    ...
    try {
      // let data = addDynamicFilter("SBU_Name", value === "PetChem" ? "PETROCHEMICALS SBU" : value);
      // let removeOld = removeOldValues(data);
  ...
  const handleZoneChange = async (event: SelectChangeEvent<string>) => {
    ...
    try {
      // let dynaFilter = addDynamicFilter("Zone_Name", value);
      // let removeOld = removeOldValues(dynaFilter);
      const keysToRemove = ['"A"', '"H"', '"T"', '"YTD"'];
      distinctFilter = newfilter.filter(
        (filter) => !keysToRemove.includes(filter.key)
      );
      const updatedFilters = distinctFilter.map((filter) => ({
        ...filter,
        key: filter.key.replace(/^"+|"+$/g, ""), // Remove leading and trailing quotes
        cond: filter.cond === "equals" ? "=" : filter.cond, // Replace "equals" with "="
      }));
  ...
  const handleRegionChange = async (event: SelectChangeEvent<string>) => {
    ...
    // setDrillHistory([...drillHistory, value]);
    try {
      // let dynaFilter = addDynamicFilter("Region_Name", value);
      // let removeOld = removeOldValues(dynaFilter);
      const keysToRemove = ['"A"', '"H"', '"T"', '"YTD"'];
      distinctFilter = newfilter.filter(
        (filter) => !keysToRemove.includes(filter.key)
  ...
  const handleSalesAreaChange = async (event: SelectChangeEvent<string>) => {
    const value = event.target.value;
    if (value === "_empty") {
      setFilters(filters.slice(0, -1));
      setDrillLevel(drillLevel - 1);
    ...
    // setDrillHistory([...new Set([...drillHistory, value])]);
    try {
      // let dynaFilter = addDynamicFilter("SalesArea_Name", value);
      // let removeOld = removeOldValues(dynaFilter);
      const keysToRemove = ['"A"', '"H"', '"T"', '"YTD"'];
      distinctFilter = newfilter.filter(
        (filter) => !keysToRemove.includes(filter.key)
  ...
  const handleProductNameChange = async (event: SelectChangeEvent<string>) => {
    const value = event.target.value;
    if (value === "_empty") {
      setFilters(filters.slice(0, -1));
      setDrillLevel(drillLevel - 1);
    ...
    // try {
    // const keysToRemove = ["\"A\"", "\"H\"", "\"T\"", "\"YTD\""];
    // distinctFilter = newfilter.filter(filter => !keysToRemove.includes(filter.key));
    // const updatedFilters = distinctFilter.map(filter => ({
    // ...filter,
    // key: filter.key.replace(/^"+|"+$/g, ""), // Remove leading and trailing quotes
    // cond: filter.cond === "equals" ? "=" : filter.cond // Replace "equals" with "="
    // }));
    // const response: any = await loadDistinctValues(["ProductName"], updatedFilters);
    // if (response) {
    // setProductOptions(response["ProductName"]);
    // }
    // } catch (error) {
  ...
  return (
    <>
      <Card className="w-full bg-white rounded-lg border border-gray-200 pt-2 px-2">
        ...
        {/* <span className="text-base font-bold">YTD (Apr 01 - Mar 31)</span>
            ...
            <div className="flex justify-between items-end">
              <CardTitle className="text-xl font-bold text-gray-800">
                <div className="flex ml-[-0.5rem] mt-1 flex-wrap gap-2 items-center">
                  {mode === "year" ? (
                    <FormControl size="small">
                      ...
                      <Select
                        ...
                        className="w-[100px] bg-white text-gray-800 border-gray-300"
                      >
                    ...
                    <FormControl size="small">
                    <InputLabel id="sbu-select-label" className="text-gray-600">
                      Company
                    </InputLabel>
                    <Select
                  ...
                  <FormControl size="small">
                    <InputLabel id="zone-select-label" className="text-gray-600">
                      Zone
                    </InputLabel>
                    <Select
                  ...
                  <FormControl size="small">
                    ...
                    <Select
                      ...
                      className="w-[130px] bg-white text-gray-800 border-gray-300"
                    >
                      <MenuItem value="_empty">All</MenuItem>
                  ...
                  <FormControl size="small">
                    ...
                    <Select
                      ...
                      className="w-32 bg-white text-gray-800 border-gray-300"
                    >
                      <MenuItem value="_empty">All</MenuItem>
              ...
              <div className="flex flex-col gap-1">
                <div className="flex gap-2">
                    ...
                    <PopoverTrigger>
                      <TooltipProvider>
                        <Tooltip>
                          <TooltipTrigger>
                            <Button
                              className={`border w-9 h-8 p-0 text-xs ${mode === "date"
                                  ? "bg-teal-600 text-white hover:bg-teal-600 hover:text-white"
                                  : "bg-white text-black hover:bg-white hover:text-black"
                                }`}
                            >
                              <Calendar strokeWidth={1} className="h-5 w-5" />
                            </Button>
                          </TooltipTrigger>
                          <TooltipContent>
                            <p>Date Range</p>
                  ...
                  {mode === "month" && (
                    // <ShadcnSelect onValueChange={handleMonthSelect}>
                    // <SelectTrigger className="w-32 h-9">
                    // <SelectValue placeholder="Select monthsss" />
                    // </SelectTrigger>
                    // <SelectContent>
                  ...
                  <div className="flex mr-2 gap-1">
                    <Button
                      ...
                      className="text-white text-xs font-bold p-1 w-8 h-8 rounded-sm shadow bg-blue-500 flex items-center justify-center hover:bg-blue-500"
                    >
      ...
      <TablePerformancesales data={chartData}
        activeStates={activeStates} drillLevel={drillLevel} />
    </>
  );
};

export default IndustrialPerformanceInsights;
