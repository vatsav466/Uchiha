...
const RegionPerformanceTable = ({ props, onRegionClick, selectedRegionName = "" }) => {
  ...
  const selectPreferredRow = () => {
    ...
    if (selectedRegionName) {
      ...
      gridApiRef.current.forEachNode((node: any) => {
        if (!matched && node.data?.Region_Name === selectedRegionName) {
  ...
  // Handle data changes — re-select the previously selected region (or first row on initial load)
  useEffect(() => {
  ...
  >
    <style>
      {`
        ...
        .region-performance-table .ag-header.ag-pivot-off.ag-header-allow-overflow {
          font-size: 12px !important;
        }
        .region-performance-table .ag-root-wrapper {
          height: 100% !important;
        }
        .region-performance-table .ag-body-viewport {
