...
const ZoneAvsTHeatmap: React.FC<Props> = ({ data, xaxisData, growthDetails, onCellClick }) => {
  ...
  const calculateYearlyGrowth = (data: ZoneData): number => {
    ...
    Object.keys(data).forEach((key) => {
      if (key.includes("actual")) {
        ...
        const targetKey = `${month}_target`;
        
        if (data[targetKey] !== undefined) {
          totalActual += data[key] as number;
          totalHistory += data[targetKey] as number;
        }
      }
    });
    ...
    const loadChart = async () => {
      ...
      data.forEach(zone => {
        ...
        months.forEach(month => {
          ...
          const target = Number(zone[`${month.split("_")[0]}_target`]) || 0;
...
export default ZoneAvsTHeatmap;