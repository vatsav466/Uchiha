...
import { ArrowDown, ArrowUp, ArrowUpDown, ArrowUpRight, Layers, RefreshCw } from "lucide-react";
...
import PASegmentDrillSheet from "./PASegmentDrillSheet";
  ...
  return (
    <div className="flex h-full min-w-0 flex-col">
      <div className="flex flex-1 flex-col overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
        <div className="flex items-start justify-between gap-2 border-b border-slate-100 px-2 py-1.5">
          <div className="flex min-w-0 flex-1 items-center gap-2">
            ...
            <div className="min-w-0">
              <h2 className="text-sm font-semibold text-slate-700">Segment split</h2>
              {!loading && rows.length > 0 && (
                <p className="mt-0.5 text-[11px] text-slate-400">
                  {rows.length} segments · {fmtTmt(total)} TMT · FY {shortFY(currentFY)} vs {shortFY(prevFY)}
                </p>
              )}
            </div>
          </div>
        ...
        {!loading && rows.length > 0 && (
          <div className={`${SEGMENT_LIST_GRID} border-b border-slate-50 px-2 py-1 text-[8px] font-semibold uppercase tracking-wide text-slate-400`}>
            <span>#</span>
            <span>Segment</span>
            <button
              type="button"
        ...
        <div className={PA_RANKED_LIST_BODY_CLASS}>
          {loading ? (
            ...
            <p className="py-6 text-center text-sm text-slate-400">No segment data</p>
          ) : (
            <div className="space-y-0.5">
              {sortedRows.map((row) => {
                ...
                return (
                  ...
                  >
                    <div className={SEGMENT_LIST_GRID}>
                      <span
