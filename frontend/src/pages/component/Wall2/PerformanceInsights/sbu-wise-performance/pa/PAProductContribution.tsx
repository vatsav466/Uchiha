...
import { ArrowDown, ArrowUp, ArrowUpRight, LayoutGrid, RefreshCw } from "lucide-react";
...
import { productDetailPayload, productMonthlyPayload } from "./pa.payloads";
...
const PAProductContribution: React.FC<Props> = ({
  ...
  const buildPayload = sheetItem
    ? (fy: string) => productDetailPayload(fy, sheetItem.name, filters, getPaDateRange(fy, compareMode))
    : null;

  const buildMonthlyPayload = sheetItem
    ? (fy: string) => productMonthlyPayload(fy, sheetItem.name, filters, getPaDateRange(fy, compareMode))
    : null;
      ...
      <div className="flex flex-1 flex-col overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
          ...
          <div className="flex min-w-0 flex-1 items-center gap-2">
            ...
            <div className="min-w-0">
              <h2 className="text-sm font-semibold text-slate-700">
                {volumeOrder === "asc" ? "Product contribution (Bottom 10)" : "Product contribution (Top 10)"}
              </h2>
              {!loading && rows.length > 0 && (
                <p className="mt-0.5 text-[11px] text-slate-400">
                  {rows.length} products · {fmtTmt(total)} TMT · FY {shortFY(currentFY)} vs {shortFY(prevFY)}
                </p>
              )}
            </div>
          </div>
        ...
        <div className={PA_RANKED_LIST_BODY_CLASS}>
          {loading ? (
            ...
            <p className="py-6 text-center text-sm text-slate-400">No product data</p>
          ) : (
...
export default PAProductContribution;
