...
const LPGPerformance: React.FC = () => {
  ...
  const ZoneWiseYTD = async () => {
    ...
    const filter = [
      ...
      { key: '"SBU_Name"', cond: "equals", value: "LPG" },
  ...
  const LPGYTDmarketingTotal = async () => {
    setIsLoading(true);
    const formatDate = (date) => {
      // console.log("data",date)
      return dayjs(date).format("YYYY-MM-DD");
  ...
  const CompactMarketingMetrics: React.FC<{ title: string; currentValue: number; historicalValue: number }> = ({
    ...
    const calculatePercentage = (currentValue: number, historicalValue: number) => {
      ...
      const v = Number((((currentValue - historicalValue) / historicalValue) * 100).toFixed(2));
    ...
    return (
      <Card className="w-full max-w-2xl shadow rounded-md bg-white border border-[#EDF4F2] bg-gradient-to-br from-purple-300 via-blue-300 to-cyan-100">
