...
import { GitBranch, Loader2, Package, RefreshCw } from "lucide-react";
...
export type ProductCategoryBreakdownPanelProps = {
  loading: boolean;
  refreshing: boolean;
...
const ProductCategoryBreakdownPanel: React.FC<ProductCategoryBreakdownPanelProps> = ({
  loading,
  refreshing,
  productCategoryRows,
  displayCurrentFY,
  ...
  const [ytdRows, setYtdRows] = useState<LubesProductCategoryRow[] | null>(null);
    ...
    try {
      ...
      const [currentRes, prevRes] = await Promise.all([
        apiClient.post(
          ...
          buildLubesProductCategoryYtdPayload(displayCurrentFY, currentDateFrom, currentDateTo, ytdExtraFilters)
        ),
        apiClient.post(
          "/api/tableanalytics/generate_data_aggregations",
          buildLubesProductCategoryYtdPayload(prevFY, prevDateFrom, prevDateTo, ytdExtraFilters)
        ),
      ]);
  ...
  const activeRows = ytdActive ? (ytdRows ?? []) : productCategoryRows;
  const activePreviousFY = ytdActive ? prevFY : displayPreviousFY;
  const isActiveLoading = ytdActive ? ytdLoading : loading;

  const chartData = useMemo(
    () => buildProductCategoryChartData(activeRows, displayCurrentFY, activePreviousFY),
  ...
  const headerAction = (
    <div className="flex items-center gap-1.5">
      ...
      <Button
        ...
        variant="ghost"
        size="icon"
        aria-label="Refresh product category data"
    ...
    >
      {isActiveLoading ? (
        ...
        <div className="py-10 text-center text-sm text-slate-500">
          No product category breakdown data available.
        </div>
      ) : (
            ...
            <div className="min-w-0">
              {filteredChartData.length === 0 ? (
                <div className="flex h-[220px] items-center justify-center text-center text-sm text-slate-500 sm:h-[260px] md:h-[280px]">
                  No data for the selected product categories.
                </div>
              ) : (
                <>
                  <BreakdownChartLegend
...
export default ProductCategoryBreakdownPanel;
