...
const IandCPerformance: React.FC = () => {
  ...
  let [fromDate, setFromDate] = useState(firstDayOfApril);
  let [toDate, setToDate] = useState(yesterday);
  ...
  const ZoneWiseYTD = async () => {
    ...
    const filter = [
      ...
      { key: '"SBU_Name"', cond: "equals", value: "I&C" },
  ...
  const IandCYTDmarketingTotal = async () => {
    setIsLoading(true);
    const formatDate = (date) => {
      // console.log("data",date)
      return dayjs(date).format("YYYY-MM-DD");
      ...
      <Card className="w-full bg-white rounded-lg border border-gray-200 pt-2 px-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-3">
          ...
          <div className="lg:col-start-3 lg:col-span-2 flex items-center justify-start">
            <span className="text-xl font-bold">I&C SBU MARKETING SUMMARY (TMT)</span>
          </div>
...
export default IandCPerformance;
