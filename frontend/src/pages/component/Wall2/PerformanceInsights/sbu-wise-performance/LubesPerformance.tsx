...
const LubesPerformance: React.FC = () => {
  ...
  const ZoneWiseYTD = async () => {
    ...
    const filter = [
      ...
      { key: '"SBU_Name"', cond: "equals", value: "Lubes" },
  ...
  const LubesYTDmarketingTotal = async () => {
    setIsLoading(true);
    const formatDate = (date) => {
      // console.log("data",date)
      return dayjs(date).format("YYYY-MM-DD");
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-3">
          <div className="lg:col-start-3 lg:col-span-2 flex items-center justify-end">
            <span className="text-lg font-bold">
              Lubes SBU MARKETING SUMMARY (TMT)
            </span>
          </div>
...
export default LubesPerformance;
