...
const SalesPerformanceChartAHTDateFilters: React.FC = () => {
  ...
  const handleSBUChange = async (value: string) => {
    ...
    //       cond: "equals",
    //       value:
    //         value === "PetChem" ? "PETROCHEMICALS SBU" : value,
    //     },
    //   ]);
    //   // setDrsillHistory([]);
    // }
  ...
  const handleSalesAreaChange = async (value: string) => {
    if (value === "_empty") {
      setFilters(filters.slice(0, -1));
      // setDrillLevel(drillLevel - 1);
      // setDrillHistory(drillHistory.slice(0, -1));
 ...
 ></div> */}
  ...
  return (
    <Card className="w-full bg-white rounded-lg border border-gray-200 pt-2 px-2">
      ...
      {/* <span className="text-base font-bold">Sales Performance Date Wise </span>
              ...
              <div className="flex items-center justify-between gap-2">
                ...
                <div className="flex gap-1">
                  <Popover open={isOpen} onOpenChange={setIsOpen}>
                    <PopoverTrigger>
                      <Button className={`border w-8 h-8 p-0 text-xs ${mode === "date" ? "bg-teal-600 text-white" : "bg-white text-black"}`}>
...
export default SalesPerformanceChartAHTDateFilters;
