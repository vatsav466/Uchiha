    ...
    <div className="bg-white shadow-lg rounded-lg p-2 border border-gray-200">
      ...
      <div className="space-y-1">
        <div className="flex justify-between">
          <span className="text-xs text-balck-600">Alert Count:</span>
...
const MetricItem = ({ label, value, color = "blue-600", isTotal = false }) => (
  <div className="flex items-center gap-3">
    ...
    <div className="flex flex-col">
      <span className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider">TOTAL(Open)</span>
...
const MainDashboardCard = ({ data, title, onSeverityFilter, onTitleClick, alertStatus, preventTitleNavigation, distributionSectionTitle, topAlertsSectionTitle }) => {
        ...
        <div className="flex">
          <div className="w-[35%] flex flex-col items-center">
            <h3 className="text-sm font-semibold text-gray-700 mb-4">
              {(data.id === 7 || data.id === 9) ? (distributionSectionTitle ?? "ITDG Alerts Distribution") : "Alerts Distribution"}
            </h3>
          ...
          <div className="w-[65%]">
            <h3 className="text-sm font-semibold text-gray-700 mb-2">{(data.id === 7 || data.id === 9) ? (topAlertsSectionTitle ?? "Top ITDG Alerts") : "Top Alerts"}
</h3>
...
interface ProjectsProps {
  ...
  /** When true, clicking the card title only filters content below (e.g. table) and does not navigate away */
  preventTitleNavigation?: boolean
  /** Optional tt_type filter for API payload (e.g. "bulk" or "packed" for VTS cards) */
  ttType?: string
  /** Optional override for (id 7/9) distribution section title, e.g. "Alert Distribution" on LPG page */
  distributionSectionTitle?: string
  /** Optional override for (id 7/9) top alerts section title, e.g. "Top Alerts" on LPG page */
  topAlertsSectionTitle?: string
}
...
const Projects = ({
  ...
  preventTitleNavigation = false,
  ttType,
  distributionSectionTitle,
  topAlertsSectionTitle,
}: ProjectsProps) => {
            ...
            <div className={mainCards.length > 1 && "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-2"}>
              {mainCards.map((card) => (
                <MainDashboardCard
                  ...
                  preventTitleNavigation={preventTitleNavigation}
                  distributionSectionTitle={distributionSectionTitle}
                  topAlertsSectionTitle={topAlertsSectionTitle}
                />
              ))}
            </div>
            {showAdditionalCards && (
